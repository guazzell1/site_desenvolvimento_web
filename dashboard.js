...
function atualizarTela() {
    const mesFiltro = document.getElementById('filtro-mes-dashboard')?.value || "";

    // Filtra as transações pelo mês selecionado
    // Se não tiver mês selecionado, usa todas
    const txFiltradas = mesFiltro
        ? transacoes.filter(t => t.data.substring(0, 7) === mesFiltro)
        : transacoes;

    const saldoAtual = txFiltradas.reduce(function(acc, t) {
    ...
    renderizarTabela(txFiltradas);
    renderizarGrafico(txFiltradas);
    renderizarGraficoLinha(txFiltradas);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Listener do filtro de mês do dashboard
    document.getElementById('filtro-mes-dashboard')?.addEventListener('change', atualizarTela);
});
